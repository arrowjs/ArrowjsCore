...
class SystemManager extends events.EventEmitter {
    ...
    setCache() {
        ...
        let data = {}
        if(self["_" + self.name]) {
            console.log(self["_" + this.name])
        }
        return this.pub.setAsync(self._config.redis_prefix + self._config.redis_key[self.name] || self.name,JSON.stringify(self))
    }
    ...
    loadComponents(name) {
            ...
            Object.keys(struc.path).map(function (id) {
                struc.path[id].path.map(function (globMaker) {
                    ...
                    listComponents.forEach(function (link) {
                        ...
                        } else if (typeof componentConfigFunction === "function"){
                            let componentConfig = componentConfigFunction();
        ...
        Object.keys(components).map(function (key) {
            ...
            let componentInfo = getInfo(components[name]);
            console.log(componentInfo);
            _.assign(components[name].info, componentInfo);
        });


        this[privateName] = components;
        this.sub.subscribe(self._config.redis_prefix + self._config.redis_event['update_' + self.name] || ('update_' + self.name));
