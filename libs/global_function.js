...
module.exports.getGlobbedFiles = function (globPatterns, removeRoot) {
  ...
  if (_.isArray(globPatterns)) {
    ...
    if (urlRegex.test(globPatterns)) {
      ...
      let files = glob.sync(globPatterns, {nosort: true});
      if (files.length > 0) {
...
function createDirectory(path) {
  fs.mkdirSync(__base + path);
}

module.exports.createArrowStructure = function createArrowStructure(folder, env) {
  const arrowFolder = path.resolve(__dirname, '..');
  nunjucks.configure(arrowFolder + '/config/template');
  fsEx.copySync(path.resolve(__dirname, '..', 'appTemplate'), folder);
  fs.mkdirSync(`${folder}/config`);

  fsEx.copySync(path.resolve(__dirname, '..', 'config/env'), folder + '/config/env');

  const configInfo = nunjucks.render('config.jst', {name: env.name, websocket: env.websocket});
  fs.writeFileSync(`${folder}/config/config.js`, configInfo);

  let dbDefaultPort = '27017';
  switch (env.db) {
    case 'mysql':
    case 'mariadb':
      dbDefaultPort = '3306';
  ...
  const dbInfo = nunjucks.render('database.jst', {type: env.db, port: dbDefaultPort});
  fs.writeFileSync(`${folder}/config/database.js`, dbInfo);
  fsEx.copySync(path.resolve(arrowFolder, 'config/error.js'), folder + '/config/error.js');
  fsEx.copySync(path.resolve(arrowFolder, 'config/express.js'), folder + '/config/express.js');
  fsEx.copySync(path.resolve(arrowFolder, 'config/redis.js'), folder + '/config/redis.js');
  fsEx.copySync(path.resolve(arrowFolder, 'config/session.js'), folder + '/config/session.js');
  fsEx.copySync(path.resolve(arrowFolder, 'config/view.js'), folder + '/config/view.js');
