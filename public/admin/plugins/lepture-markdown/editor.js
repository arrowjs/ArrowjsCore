...
// CodeMirror is the only global var we claim
    ...
    var callDialog = function (title, defaultInputText, callback) {
        ...
        var createDialog = function () {
            ...
            // <div class="modal fade" id="vhc-lepture-modal">
            ...
            // The iframe integrate with roxy fileman
            if (title == 'Insert Image') {
                modal_dialog.style.width = '80%';
                iframe = doc.createElement('iframe');
                iframe.setAttribute("src", "/fileman/index.html?integration=custom&type=files&leptureFieldId=imgURL");
            ...
            var el = document.getElementsByClassName('CodeMirror')[0].parentElement;
            var isFull = (el.clientWidth == screen.width);
            if(isFull) dialog.style.zIndex = 2147483647;
    ...
    function toggleFullScreen(editor) {
        ...
        if(!safari) {
            ...
            $(window).on('resize', function (e) {
                ...
                resizeTimer = setTimeout(function () {
                    ...
                    if (!(el.clientWidth == screen.width)) {
                        ...
                        if (preview.length > 0) {
                            $('.CodeMirror-lines').css('width', '100%');
                            if (preview.hasClass('editor-preview-active'))
                                preview.removeClass('editor-preview-active');
                            if (preview.hasClass('preview-fullscreen'))
                                preview.removeClass('preview-fullscreen');
                        }
                    }
                }, 250);
            });
            ...
            var request = function () {
                ...
                $('.CodeMirror').addClass('CodeMirror-full');

                editor.togglePreview();
                preview = $('.editor-preview');

                preview.addClass('preview-fullscreen');
                $('.CodeMirror-lines').css('width', '50%');
                ...
                $('.CodeMirror-scroll').on('scroll', function () {
                    $('.preview-fullscreen').scrollTop($(this).scrollTop());
                });
                //this code make scroll slow on firefox
                //$('.editor-preview').on('scroll', function () {
            ...
            var cancel = function () {
                ...
                $('.CodeMirror').removeClass('CodeMirror-full');
                preview = $('.editor-preview');
                if (preview.length > 0) {
                    $('.CodeMirror-lines').css('width', '100%');
                    if (preview.hasClass('editor-preview-active'))
    ...
    function drawLink(editor) {
        var cm = editor.codemirror;
        callDialog('Insert link', 'http://', function (url) {
            if (url) {
                var stat = getState(cm);
    ...
    function drawImage(editor) {
        var cm = editor.codemirror;
        callDialog('Insert Image', 'Choose image', function (url) {
            if (url) {
                var stat = getState(cm);
