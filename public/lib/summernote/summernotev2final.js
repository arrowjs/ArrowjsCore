 ...
 *
 * Customize by Vu Hoang Chung - http://techmaster.vn
 * Add Striped Table (Boostrap theme), Highlight syntax (using prism.js)
 *
 * Date: 16/01/2015
 */
(function (factory) {
    ...
    var settings = {
        ...
        options: {
            ...
            //code tag for vhcsyntax
            codeTags: [
                'none', 'c', 'cpp', 'csharp', 'markup', 'css',
                'jade', 'twig', 'javascript',
                'php', 'sql', 'java',
                'ruby', 'ASP.NET', 'objectivec', 'swift'
            ],
    ...
    var StripTable = function () {
        ...
        this.createTable = function (colCount, rowCount) {
            ...
            trHTML = trs.join('');
            return $('<table class="table table-bordered table-striped">' + trHTML + '</table>')[0];
        };
    };
    ...
    var Editor = function () {
         ...
         * @param {String} lang
         */
        this.formatCode = function ($editable, lang) {
            var range = window.getSelection().getRangeAt(0);
            var selectionContents = range.extractContents();
            parentEl = range.commonAncestorContainer;
            /**
             * http://www.w3schools.com/dom/dom_nodetype.asp
             * check nodeType. type 1 == element node
             */
            ...
            if (lang == 'none') {
                if (parentEl.nodeType == 1) {
                    if (parentEl.nodeName.toLowerCase() == 'code') {
                        parentEl = parentEl.parentNode;
                        $(parentEl).remove();
                        p.appendChild(selectionContents);
                        range.insertNode(p);
                    } else {
                        if (parentEl.className == 'note-editable') {
                            $('.note-editable').find('pre').remove();
                        }
                        p.appendChild(selectionContents);
                        range.insertNode(p);
                    }
                    ...
                    //find <pre> tag
                    if (parentEl.nodeName.toLowerCase() == 'code') {
                        var parent = $(parentEl).parent();
                        //remove prism tag and pre tag
                        $(parentEl).remove();
                        $(parent).remove();
                        p.appendChild(selectionContents);
                        range.insertNode(p);
                        ...
                        p.appendChild(selectionContents);
                        range.insertNode(p);
                        if (parentEl.nodeName.toLowerCase() == 'p') {
                            $(p).unwrap();
                        }
                    }
                }
                ...
                code.appendChild(selectionContents);
        ...
        //vhc insert striped table
        this.insertStripTable = function ($editable, sDim) {
            var dimension = sDim.split('x');
            var rng = range.create();
            rng = rng.deleteContents();
            rng.insertNode(stripTable.createTable(dimension[0], dimension[1]));
            afterCommand($editable);
        };
    ...
    var Renderer = function () {
        ...
        var tplButtonInfo = {
            ...
            //highlight syntax by Vu Hoang Chung - techmaster.vn @2015
            vhcsyntax: function (lang, options) {
                var items = options.codeTags.reduce(function (memo, v) {
                    //var label = lang.style[v === 'p' ? 'normal' : v];
                    var label = lang.vhcsyntax[v === 'ASP.NET' ? 'aspnet' : v];
                    return memo + '<li><a data-event="formatCode" href="#" data-value="' + v + '">' + label + '</a></li>';
                }, '');

                return tplIconButton('fa fa-file-code-o', {
                    title: lang.vhcsyntax.syntax,
                    dropdown: '<ul class="dropdown-menu">' + items + '</ul>'
                });
            },
            ...
            stripTable: function (lang) {
                var dropdown = '<ul class="note-table dropdown-menu">' +
                    '<div class="note-dimension-picker">' +
                    '<div class="note-dimension-picker-mousecatcher" data-event="insertStripTable" data-value="1x1"></div>' +
                    '<div class="note-dimension-picker-highlighted"></div>' +
                    '<div class="note-dimension-picker-unhighlighted"></div>' +
                    '</div>' +
                    '<div class="note-dimension-display"> 1 x 1 </div>' +
